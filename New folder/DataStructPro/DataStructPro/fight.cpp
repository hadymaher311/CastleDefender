...
void heal(castle &ct, int index, int i, enemy *&mainTemp) {
	if (mainTemp->RemainingTime == 0) {
		enemy* temp = ct.towers[i].active;
		if (mainTemp->power > 1) mainTemp->power = 1;

		if (index == 0)
		{
			temp = NULL;
			mainTemp->RemainingTime = mainTemp->reloadPriod;
			return;
		}

		else if (index == 1) {
			if (temp->Type != 5) {
				temp->Health = temp->Health + mainTemp->power;
				temp = NULL;
				mainTemp->RemainingTime = mainTemp->reloadPriod;
				return;
			}
