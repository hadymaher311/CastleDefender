...
void Out(castle ct, enemy *Killed, enemy *head, double TH)
{
	...
	while (k)
	{
		outfile << k->killDelay+k->fightDelay+k->timeArrival << "\t" << k->ID << "\t" << k->fightDelay << "\t" << k->killDelay << "\t" << k->fightDelay+k->killDelay << "\n";
		k = k->next;
	}
	...
	else
	{
		...
		outfile << "Total Alive Enemies\t= " << counter + countList(head) << "\n";		//Print The Number Of Alive Enemies
		outfile << "Average Fight Delay\t= " << Avarage(Killed, 'f') << "\n";
		outfile << "Average Kill Delay\t= " << Avarage(Killed, 'k') << "\n";
	}
	delete k;
