...
void activeList(enemy *&head, enemy*&last, castle&ct, int timeStep)		//Make The Active List each timestep
{
	enemy *temp = head;
	while (temp != NULL && temp->timeArrival == timeStep)
	{
		...
		temp = temp->next;
		remove_from_beg(head,last);																	//Removing This Enemy From The Inactive List
	}
}
