...
void Simulation(char Number)
{
	...
	enemy *head, *last, *killedHead, *killedLast;				//Inactive List And Killed List
	head = last = killedHead = killedLast = NULL;

	inputfromfile(head, last, C1, C2, C3, TH, N, TP);			//Take Input From File
	...
	for (int i = 0; i < 4; i++)			//intialize all parametars of towers
	{
		...
		ct.towers[i].active = ct.towers[i].active = ct.towers[i].high = ct.towers[i].highlast = NULL;
		ct.towers[i].Numberofkilled = 0;
		ct.towers[i].TPaved = 30;
	}

	ct.totalNumberofEnemies = last->ID;  //Detect Total Number of enemies from last element in inActive list


	int TimeStep = 0;		//Set The Time Step To 0

	///Now a Simulation
	while (GameOver(ct, head))											//While Loop until All towers are killed or all enemies
	{
		TimeStep++;														//Increment The Time Step

		checkTowers(ct, head);											// If tower health = 0  move enemies to next region clock wise

		DecreseDistance(ct);											//Decrease The Distance Of The Enemies

		activeList(head, last, ct, TimeStep);							//Add The Enemies Whose Time Step = Current Time Step To The Active List According To His Region		
	...
	Out(ct, killedHead, head, TH);				//Make The Output File
